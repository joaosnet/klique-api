...
async def get_radar(
    ...
    domains_docs = await domains_col.find({'user_id': user_id}).to_list(length=None)
    result = []
...
async def get_activity(
    ...
    pipeline = [
        ...
        {'$project': {
            'day': {'$dateToString': {'format': '%Y-%m-%d', 'date': '$reviewed_at'}},
    ...
    docs = await logs_col.aggregate(pipeline).to_list(length=None)
    ...
    for i in range(7):
        ...
        result.append(activity_map.get(day_str, ActivityEntry(date=day_str, count=0, avg_rating=0.0)))
