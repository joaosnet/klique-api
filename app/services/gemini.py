        ...
        try:
            ...
            generate_content_config = types.GenerateContentConfig(
                ...
                response_schema=genai.types.Schema(
                    ...
                    required=['prompt_aprimorado'],
                    properties={
                        'prompt_aprimorado': genai.types.Schema(
...
- Fale naturalmente: “remova o logo” e “pinte o moletom de roxo” funcionam perfeitamente."""  # noqa: E501
            ...
            for chunk in response_stream:
                if hasattr(chunk, 'text') and chunk.text:
                    resposta_json += chunk.text

            try:
                resposta = json.loads(resposta_json)
                aprimorado = resposta.get('prompt_aprimorado')
                if aprimorado:
                    logger.info('Prompt aprimorado com sucesso.')
                    return aprimorado
                else:
                    logger.warning(
                        'Campo "prompt_aprimorado" não '
                        'encontrado na resposta JSON.'
                    )
                    return prompt
            except Exception as e:
