...
logger.add("execution_logs.log", rotation="1 MB", retention="10 days", level="DEBUG")
...
def auto_detect_watermark(image_path, template_path=None, threshold=0.7):
    ...
    if not os.path.exists(template_path):
        logger.warning(f"Template '{template_path}' não encontrado. Automação pulada.")
        return None
    ...
    logger.debug(f"Confiança da detecção: {max_val:.4f} (Threshold: {threshold})")
    ...
    try:
        ...
        with Image.open(input_path) as img:
            ...
            if not output_path:
                ...
                output_path = f"{base}_nobg.png"  # PNG para preservar transparência
...
def get_realesrgan_upsampler(scale: int = 4, model_name: str = "RealESRGAN_x4plus"):
    """
    ...
    try:
        ...
        if not os.path.exists(model_path):
            logger.info(f"Modelo '{model_name}' não encontrado localmente. Baixando de {model_url} ...")
            try:
                ...
                raise FileNotFoundError(f"Modelo '{model_name}' não encontrado e falha ao baixar: {e}")
...
def upscale_realesrgan(
    ...
    try:
        ...
        # Obter upsampler
        upsampler = get_realesrgan_upsampler(scale=scale, model_name=model_name)
    ...
    try:
        ...
        if img is not None:
            ...
            if sharpen_strength > 0:
                logger.debug(f"Aplicando sharpening (força: {sharpen_strength})...")
                # Unsharp mask (mais natural que kernel simples)
                gaussian = cv2.GaussianBlur(img, (0, 0), 3)
                img = cv2.addWeighted(img, 1 + sharpen_strength, gaussian, -sharpen_strength, 0)
...
def show_debug_window(input_path, box_w, box_h, off_x, off_y):
    ...
    try:
        ...
        # Botão fechar
        ttk.Button(debug_win, text="Fechar", command=debug_win.destroy).pack(pady=5)
...
def open_gui():
    ...
    style_val = {"bg": "#333", "fg": "white", "font": ("Consolas", 10), "width": 8}
    style_btn = {
        "bg": "#444",
        "fg": "white",
        "relief": "flat",
    ...
    def select_file():
        ...
        if filename:
            ...
            if detected:
                ...
                status_var.set("Autodetecção: Marca encontrada! Confiança alta.")
                messagebox.showinfo(
                    "V's AutoScan",
        ...
        def on_complete(status, data):
            if status == "success":
                ...
                if out:
                    ...
                    status_var.set(f"✓ Fundo removido: {os.path.basename(out)}")
                else:
                    messagebox.showerror("Erro", msg)
    ...
    def run_upscale_realesrgan():
        ...
        def on_complete(status, data):
            if status == "success":
                out, msg = data
                if out:
                    messagebox.showinfo("Sucesso", f"Real-ESRGAN concluído!\n{os.path.basename(out)}")
                    status_var.set(f"✓ Real-ESRGAN {scale}x: {os.path.basename(out)}")
                else:
                    messagebox.showerror("Erro", msg)
                    status_var.set(f"✗ Erro: {msg}")
            else:
    ...
    def run_full_pipeline():
        ...
        def pipeline_worker(input_path, alpha_matting, scale_factor, sharpen, denoise, face_enhance):
            """Worker que executa o pipeline completo."""
            results = []
            current_file = input_path

            # 1. Remover fundo
            out, msg = remove_background(current_file, alpha_matting=alpha_matting)
            if out:
        ...
        run_in_background(
            pipeline_worker,
            args=(fpath, var_alpha_matting.get(), scale, var_sharpen.get(), var_denoise.get(), var_face_enhance.get()),
            kwargs={},
    ...
    frame_watermark = tk.LabelFrame(
        root, text="🔲 Remoção de Marca D'água", bg="#1e1e1e", fg="white", padx=10, pady=10
    )
    ...
    tk.Entry(frame_ctrl, textvariable=var_width, **style_val).grid(row=0, column=1)
    tk.Label(frame_ctrl, text="H:", **style_lbl).grid(row=0, column=2)
    tk.Entry(frame_ctrl, textvariable=var_height, **style_val).grid(row=0, column=3)
    tk.Label(frame_ctrl, text="OffX:", **style_lbl).grid(row=1, column=0)
    tk.Entry(frame_ctrl, textvariable=var_off_x, **style_val).grid(row=1, column=1)
    tk.Label(frame_ctrl, text="OffY:", **style_lbl).grid(row=1, column=2)
    tk.Entry(frame_ctrl, textvariable=var_off_y, **style_val).grid(row=1, column=3)
    ...
    tk.Button(
        ...
        text="DEBUG",
        command=lambda: show_debug_window(
            file_path_var.get(),
            var_width.get(),
            var_height.get(),
            var_off_x.get(),
    ...
    frame_bg = tk.LabelFrame(
        root, text="🎨 Remoção de Fundo (IA)", bg="#1e1e1e", fg="white", padx=10, pady=10
    )
    ...
    frame_upscale = tk.LabelFrame(
        root, text="📐 Upscale Real-ESRGAN (IA Profissional)", bg="#1e1e1e", fg="white", padx=10, pady=10
    )
    ...
    tk.Label(frame_upscale_opts, text="Escala:", **style_lbl).grid(row=0, column=0)
    ...
    tk.Label(frame_upscale_opts2, text="Sharpening:", **style_lbl).pack(side="left")
    ...
    frame_pipeline = tk.LabelFrame(
        root, text="🚀 Pipeline Completo", bg="#1e1e1e", fg="white", padx=10, pady=10
    )
    ...
    tk.Label(root, textvariable=status_var, **style_lbl).pack(side="bottom", pady=5)
