...
// Prisioner's Dilemma logic (same as original HTML)
function computeMatch(p1, p2) {
  if (p1.strategy === 'withdraw' || p2.strategy === 'withdraw') {
    return { type: 'neutral', synergyDelta: 0, msg: 'Nenhuma interação. Um lado se isolou.' };
  }
  if (p1.strategy === 'cooperate' && p2.strategy === 'cooperate') {
    return { type: 'win', synergyDelta: 2, msg: 'Sinergia! Ambos colaboraram e o sistema prosperou.' };
  }
  if (p1.strategy === 'defect' && p2.strategy === 'defect') {
    return { type: 'lose', synergyDelta: -2, msg: 'Guerra de Atrito! Ambos tentaram se explorar e perderam recursos.' };
  }
  ...
  return {
    type: 'exploit',
    synergyDelta: -1,
    msg: `${exploiter} explorou a colaboração de ${victim}. O traidor lucra sozinho.`,
  };
}

const TYPE_STYLE = {
  win:     { border: 'border-blue-800',   tag: 'bg-blue-900 text-blue-400 border-blue-500',   label: 'Ganho Mútuo' },
  lose:    { border: 'border-red-800',    tag: 'bg-red-900 text-red-400 border-red-500',      label: 'Destruição Mútua' },
  exploit: { border: 'border-orange-800', tag: 'bg-orange-900 text-orange-400 border-orange-500', label: 'Exploração' },
  neutral: { border: 'border-gray-700',   tag: 'bg-gray-800 text-gray-400 border-gray-600',   label: 'Isolamento' },
};
...
export default function SimulatorResults({ population, onReset }) {
  ...
  while (poolCopy.length >= 2) {
    ...
    const result = computeMatch(p1, p2);
    totalSynergy += result.synergyDelta;
    matchCount++;
    matches.push({ p1, p2, ...result });
  }
  if (poolCopy.length === 1) {
    matches.push({ p1: poolCopy[0], p2: null, type: 'neutral', msg: 'Ficou sem par nesta rodada.' });
  }
  ...
  const scoreLabel =
    synergyScore >= 70 ? 'Ecossistema Próspero' :
    synergyScore >= 40 ? 'Ambiente Instável' :
    'Colapso do Sistema (Tóxico)';
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between mb-4 flex-shrink-0 pb-3"
        ...
        <div>
          <h2 className="font-title text-xl text-yellow-500 tracking-wider">RESULTADOS</h2>
          <p className="text-xs text-gray-400 uppercase tracking-widest mt-0.5">
            {matches.length} confrontos simulados
          </p>
        </div>
      ...
      >
        <p className="text-xs text-gray-300 leading-relaxed">
          <span className="font-bold" style={{ color: scoreColor }}>Índice de Sinergia: {synergyScore}%</span>
          {' — '}Baseado nas estratégias do Dilema do Prisioneiro.
          Cooperação gera +2, exploração −1, guerra de atrito −2, isolamento 0.
        </p>
      </div>
