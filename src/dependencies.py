...
# In a real-world application, you would use a more persistent
# and scalable solution like Redis or a database.
...
async def authenticate_token(token: str, db_users):
    try:
        payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
        username: str = payload.get('sub')
        if username is None:
