...
def test_image_modification_conversation():
    """
    Testa uma conversa de múltiplos turnos com a API para manipulação de imagens,
    ...
    with TestClient(app) as client:
        ...
        assert response1.status_code == 200, f"A primeira requisição falhou com o status {response1.status_code}: {response1.text}"
        response_data1 = response1.json()
        assert "session_id" in response_data1, "A resposta deve conter um session_id."
        assert (
            "generated_image" in response_data1
        ), "A resposta deve conter uma imagem gerada."
        assert (
            response_data1["generated_image"] is not None
        ), "A imagem gerada não pode ser nula."

        session_id = response_data1["session_id"]

        # 4. Passo 2: Continuar a conversa com um segundo prompt, usando o session_id
        ...
        assert response2.status_code == 200, f"A segunda requisição falhou com o status {response2.status_code}: {response2.text}"
        response_data2 = response2.json()
        assert (
            response_data2["session_id"] == session_id
        ), "O session_id deve ser o mesmo na continuação da conversa."
        assert (
            "generated_image" in response_data2
        ), "A segunda resposta também deve conter uma imagem."
        assert (
            response_data2["generated_image"] is not None
